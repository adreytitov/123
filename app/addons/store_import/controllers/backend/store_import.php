...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (!empty($_REQUEST['import_settings'])) {
        $params_list = array();
        foreach ($_REQUEST['import_settings'] as $param => $value) {
            if ($value == 'N' || $value == 'Y') {
                $params_list[$param] = ($value == 'Y');
            }
        }
        General::updateStoreimportSetting($params_list);
    }
    if ($action == 'step_1') {
        $step = 1;
        list($is_valid_step, $valid_step) = General::checkStep($step);
        if (!$is_valid_step) {
            return array(CONTROLLER_STATUS_REDIRECT, 'store_import.index.step_' . $valid_step);
        }
        ...
        Tygh::$app['view']->assign('step4_notification', $step4_notification);
    } elseif ($action == 'step_5') {
        list($is_valid_step, $valid_step) = General::checkStep(5);
        if (!$is_valid_step) {
            return array(CONTROLLER_STATUS_REDIRECT, 'store_import.index.step_' . $valid_step);
        }
        $result = true;
        if (!General::testDatabaseConnection($store_data)) {
            ...
            Tygh::$app['view']->assign('check_company_count_failed', true);
        }
        if ($result) {
            $step = 5;
        } else {
        ...
        if (defined('AJAX_REQUEST')) {
            ...
            Tygh::$app['ajax']->assign('force_redirection', fn_url('store_import.index.step_5'));
        }
...
if ($step == '2') {
    if (!General::testDatabaseConnection($store_data)) {
        ...
        Tygh::$app['view']->assign('companies_count_from', count(General::getCompanies($store_data)));
    }
...
if ($step == '3') {
    ...
    if ($import_result) {
        ...
        if (defined('AJAX_REQUEST')) {
            Tygh::$app['ajax']->assign('non_ajax_notifications', true);
            Tygh::$app['ajax']->assign('force_redirection', fn_url('store_import.index.step_4'));
        }
        ...
        return array(CONTROLLER_STATUS_REDIRECT, 'store_import.index');
    }
}
...
if ($step !== 1) {
    ...
    Tygh::$app['view']->assign('text_from', $text_from);
    Tygh::$app['view']->assign('text_to', $text_to);
}
Tygh::$app['view']->assign('step', $step);
Tygh::$app['view']->assign('import_date', $si_data['import_date']);
Tygh::$app['view']->assign('store_data', $store_data);
Tygh::$app['view']->assign('steps', array(
    '1' => array(
