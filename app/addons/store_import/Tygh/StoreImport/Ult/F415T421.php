...
class F415T421
{
    ...
    /**
     * Concatenates deprecated 'text_thumbnail_manual_loading' and 'thumbnail_manual_loading_link' langvars
     *
     * @throws \Tygh\Exceptions\StoreImportException
     */
    protected function concatThumbnailVariables()
    {
        General::connectToImportedDB($this->store_data);
        $new_values = array();
        foreach (General::getLangCodes() as $lang_code) {
            $old_values = db_get_hash_single_array(
                "SELECT name, value"
                ...
                array('name', 'value'),
                $lang_code,
                array('text_thumbnail_manual_loading', 'thumbnail_manual_loading_link')
            );
            if (!empty($old_values)) {
                $new_values[$lang_code] = "{$old_values['text_thumbnail_manual_loading']} <a id=\"[id]\" class=\"[class]\">{$old_values['thumbnail_manual_loading_link']}</a>";
            }
        }
        General::connectToOriginalDB(array('table_prefix' => General::formatPrefix()));
        foreach ($new_values as $lang_code => $value) {
            db_query(
