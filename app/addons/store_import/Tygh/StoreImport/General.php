...
class General
{
    ...
    {
        if (db_connect_to(array('dbc_name' => self::CONNECTION_NAME), $store_data['db_name'])) {
            return true;
        } else {
    ...
    public static function testDatabaseConnection($store_data)
    {
        ...
        if (!empty($store_data['db_host']) && !empty($store_data['db_user']) && !empty($store_data['db_name'])) {
            $new_db = @self::initiateImportedDB($store_data);
    ...
    {
        $status = false;
        self::connectToImportedDB($store_data);
    ...
    public static function getConfig($cart_path)
    {
        ...
        if (file_exists($cart_path . '/config.local.php') && file_exists($cart_path . '/config.php')) {
            ...
            $config_local_php =  self::removePhpComments($config_local_php);

            $config['db_host'] = self::getVariable('db_host', $config_local_php);
            $config['db_name'] = self::getVariable('db_name', $config_local_php);
            $config['db_user'] = self::getVariable('db_user', $config_local_php);
            $config['db_password'] = self::getVariable('db_password', $config_local_php);
            $config['table_prefix'] = self::getConstant('TABLE_PREFIX', $config_local_php);
            if (empty($config['table_prefix'])) {
                $config['table_prefix'] = self::getVariable('table_prefix', $config_local_php);
            }
            $config['storefront'] = self::getVariable('http_host', $config_local_php) . self::getVariable('http_path', $config_local_php);
            $config['secure_storefront'] = self::getVariable('https_host', $config_local_php) . self::getVariable('https_path', $config_local_php);

            $config['crypt_key'] = self::getVariable('crypt_key', $config_local_php);
            $config['admin_index'] = $config['storefront'] . '/' . self::getVariable('admin_index', $config_local_php);

            //Read settings from config.php
            $config_php = file_get_contents($cart_path . '/config.php');
            $config_php =  self::removePhpComments($config_php);
            $config['product_edition'] = self::getConstant('PRODUCT_EDITION', $config_php);
            if (empty($config['product_edition'])) { // workaround for all versions, where edition was stored in PRODUCT_TYPE const
                $config['product_edition'] = self::getConstant('PRODUCT_TYPE', $config_php);
            }
            $config['product_version'] = self::getConstant('PRODUCT_VERSION', $config_php);
            $config['product_name'] = self::getConstant('PRODUCT_NAME', $config_php);
     ...
     */
    private static function removePhpComments($code)
    {
        return preg_replace("%//.*?\n%is", '', preg_replace("%/\*(.*?)\*/\s+%is", '', preg_replace("%^\s*?#.*?\n%ism", '', $code)));
    }
     ...
     */
    private static function getVariable($var_name, $config)
    {
     ...
     * @return string Variable value
     */
    private static function getConstant($var_name, $config)
    {
    ...
    private static function getImportClassName($str, $edition)
    {
    ...
    public static function getImportSchema($store_data)
    {
        ...
        $product_version = self::getVersionName($store_data['product_version']);
    ...
    /**
     * Returns list of class names that should be consequently used for import
     *
     * @param array $store_data Parameters of imported and importing stores
     *
     * @return array Names of import classes
     */
    public static function getImportClassesCascade($store_data)
    {
        ...
        if (!empty($import_schema)) {
            ...
            foreach ($import_schema as $value) {
                $return[] = self::getImportClassName($value, $edition);
            }
        }
    ...
    private static function getVersionName($product_version)
    {
    ...
    public static function cloneImportedDB($store_data)
    {
        ...
        self::connectToImportedDB($store_data);
        $tables = self::getTables($store_data['db_name'], $store_data['table_prefix']);
        ...
        db_export_to_file(Registry::get('config.dir.backups') . 'export.sql', $tables, true, true, false, true, true, $change_table_prefixes);

        self::connectToOriginalDB();
        self::createExcludedTables($change_table_prefixes['to']);

        $import_result = db_import_sql_file(Registry::get('config.dir.backups') . 'export.sql', 16384, true, true, false, false, false, true);
        if ($import_result) {
            self::updateSystemTablesStructure();
        }
    ...
    private static function createExcludedTables($prefix)
    {
        db_query("
    ...
    private static function replacePrefix($str, $table_prefix)
    {
    ...
    {
        self::connectToOriginalDB(array('table_prefix' => self::formatPrefix()));
    ...
    public static function uninstallAddons($addons = array())
    {
        if (!empty($addons)) {
            ...
            self::removeAddonsSettings($addons);
        }

        return true;
    }

    private static function removeAddonsSettings($addons)
    {
        ...
        if (!isset($table_fields['options'])) {
            ...
            if (!empty($addons_objects)) {
                db_query("DELETE FROM ?:settings_descriptions WHERE object_id IN (?n) AND object_type = 'O'", $addons_objects);
                db_query("DELETE FROM ?:settings_variants WHERE object_id IN (?n)", $addons_objects);
                if (db_get_array("SHOW TABLES LIKE '?:settings_vendor_values'")) {
                    db_query("DELETE FROM ?:settings_vendor_values WHERE object_id IN (?n)", $addons_objects);
                }
            }
        }
    ...
    {
        $addons = empty($addons) ? self::getInstalledAddons() : $addons;
        ...
        foreach ($addons as $addon) {
            $sql_filename = Registry::get('config.dir.addons') . 'store_import/database/addons/' . self::getFileName($store_data, $class_name, $addon) . '.sql';
            $php_filename = Registry::get('config.dir.addons') . 'store_import/scripts/addons/' . self::getFileName($store_data, $class_name, $addon) . '.php';

            if (is_file($sql_filename)) {
                Logger::instance()->write(self::getUnavailableLangVar(
                    'addon_sql_import',
                    array(
                ...
                $is_successfull = true;
                try {
                    db_import_sql_file($sql_filename, 16384, false, true, false, false, false, false);
                } catch (DatabaseException $db_exception) {
                    $is_successfull = false;
                    self::logError(self::ERROR_SQL, $db_exception->getMessage());
                }
                if (!$is_successfull) {
                    return false;
                }
            ...
            if (is_file($php_filename)) {
                ...
                set_error_handler(array('Tygh\\StoreImport\\General', 'logError'));
    ...
    {
        $version_name = self::getVersionName($store_data['product_version']);
    ...
    public static function replaceOriginalDB($store_data, $enable_non_imported_objects = false)
    {
        ...
        $new_table_prefix = self::formatPrefix();
        ...
        foreach ($orig_tables as $table_name) {
            ...
            fn_set_progress('echo', '<br/>' . self::getUnavailableLangVar('dropping_original_table', array('[table]' => $table_name)), true);
        ...
        foreach ($imported_tables as $table_name) {
            ...
            fn_set_progress('echo', '<br/>' . self::getUnavailableLangVar('renaming_original_table', array('[table]' => $table_name)), true);
    ...
    public static function convertPresets401To402()
    {
        ...
        foreach ($presets as $preset) {
            $preset['name'] = self::formPresetName($preset['name']);
    ...
    private static function formPresetName($name)
    {
    ...
    public static function testStoreConfiguration($store_data)
    {
        ...
        if ($config_local_php) {
            $cache_backend = self::getVariable('cache_backend', $config_local_php);
...
//            return false;
        ...
        if (!empty($store_data)) {
            ...
            if (isset($mapping[$orig_version]) && in_array(strtolower($edition_str), $mapping[$orig_version])) {
                return true;
            }
        }
            ...
            if (!empty($addon_scheme)) {
                ...
                if (!empty($tabs)) {
                    foreach ($tabs as $tab_index => $tab) {
                        $addon_section_id = db_get_field("SELECT section_id FROM ?:settings_sections WHERE name = ?s AND parent_id = (SELECT section_id FROM ?:settings_sections WHERE name = ?s AND type = 'ADDON')", $tab['id'], $addon);
...
/*
        db_query("INSERT INTO ?:state_descriptions (SELECT state_id, ?s as lang_code, state FROM " . $prefix . "state_descriptions WHERE lang_code = ?s) ON DUPLICATE KEY UPDATE ?:state_descriptions.state_id = ?:state_descriptions.state_id", self::$default_language, DEFAULT_LANGUAGE);
        if (fn_allowed_for('ULTIMATE')) {
            db_query("INSERT  INTO ?:ult_language_values (SELECT ?s as lang_code, name, value, company_id FROM " . $prefix . "ult_language_values WHERE lang_code = ?s) ON DUPLICATE KEY UPDATE ?:ult_language_values.value = ?:ult_language_values.value", self::$default_language, DEFAULT_LANGUAGE);
        }
 ...
 */
        ...
        foreach ($langs as $lang_code) {
            ...
            if (!is_dir($po_path . $lang_code)) {
                ...
                if (is_dir($po_path . self::$default_language)) {
                    $lang_code = self::$default_language;
                    $params['force_lang_code'] = self::$default_language;
    ...
    public static function setEmptyProgressBar($text = '', $scale_count = 1)
    {
        ...
        fn_set_progress('echo', '<br/>' . $text, true);
    }

    private static function removeTempTables()
    {
        $prefix = self::formatPrefix();
    ...
    private static function setUnavailableLangVars()
    {
        self::$unavailable_lang_vars = array(
            ...
            'sql_error' => __('addons.store_import.sql_error'),
            'addon_sql_import' => __('addons.store_import.addon_sql_import'),
            'addon_script_run' => __('addons.store_import.addon_script_run'),
            'script_error' => __('addons.store_import.script_error'),
            'dropping_original_table' => __('addons.store_import.dropping_original_table'),
            'renaming_original_table' => __('addons.store_import.renaming_original_table'),
            'errors_during_import' => __('addons.store_import.errors_during_import', array('[log_dir]' => Registry::get('config.dir.store_import')))
        );
    }

    public static function getUnavailableLangVar($name, $params = array())
    {
        $langvars = self::$unavailable_lang_vars;

        if (!empty($langvars[$name]) && !empty($params)) {
            $langvars[$name] = strtr($langvars[$name], $params);
        }

        return !empty($langvars[$name]) ? $langvars[$name] : '';
    }

    private static function uninstallAllAddons()
    {
    ...
    public static function import($store_data, $actualize_data = false)
    {
        ...
        $source_store = array(
            'version' => $store_data['product_version'],
            'edition' => $store_data['product_edition'],
            'storefront' => $store_data['storefront'],
            'secure_storefront' => $store_data['secure_storefront'],
        );
        $destination_store = array(
            'license_key' => self::getLicenseNumber(array('product_version' => PRODUCT_VERSION)),
            'version' => PRODUCT_VERSION,
            'edition' => PRODUCT_EDITION,
            'storefront' => $store_data['new_storefront_url'],
            'secure_storefront' => $store_data['new_secure_storefront_url']
        );
        ...
        $style_id_to_be_installed = db_get_field("SELECT style_id FROM ?:bm_layouts WHERE is_default = '1' AND theme_name = '$theme_to_be_installed'");
        set_error_handler(array('Tygh\\StoreImport\\General', 'logError'));
        register_shutdown_function(array('Tygh\\StoreImport\\General', 'shutdown'));
        ...
        if ($result) {
            ...
            if (fn_allowed_for('ULTIMATE')) {
                self::ultProcessImages($store_data);
            } else {
                self::mveProcessImages($store_data);
            }

            if (self::getImportSettings('copy_downloads', true)) {
                self::processFiles($store_data, 'downloads');
            }
            if (self::getImportSettings('copy_attachments', true)) {
                self::processFiles($store_data, 'attachments');
            }
            if (self::getImportSettings('copy_custom_files', true)) {
                self::processFiles($store_data, 'custom_files');
            }

            fn_clear_cache();

            if (!$actualize_data) {
                self::removeRussianServices($store_data);
                self::uninstallAddons(array('twigmo', 'searchanise', 'live_help', 'exim_store', 'webmail'));
                self::replaceImportedTheme($theme_to_be_installed, $style_id_to_be_installed);
            }
            ...
            if (defined('AJAX_REQUEST')) {
                ...
                \Tygh::$app['ajax']->assign('force_redirection', fn_url('index.index'));
            }
            if ($actualize_data) {
                self::restoreLogos($logos);
            }

            self::reportStoreImport($source_store, $destination_store, $actualize_data);

            fn_set_progress('step_scale', '1');
            fn_set_progress('echo', '<br/>' . __('store_import.done'), true);
    ...
    private static function installTheme($company_id = null)
    {
        $theme_name = 'basic';
        $style = 'satori';
    ...
    public static function setLicenseData()
    {
        ...
        if (!empty($si_data['license_data'])) {
            ...
            \Tygh::$app['session']['last_status'] = 'ACTIVE';
            \Tygh::$app['session']['mode_recheck'] = false;
    ...
    public static function copyProductsBlocks($store_data)
    {
        ...
        $descriptions_22x = db_get_hash_multi_array("SELECT * FROM ?:block_descriptions WHERE block_id IN (?n)", array('block_id', 'lang_code'), array_keys($blocks_22x));
        $blocks_links_22x = db_get_hash_multi_array("SELECT * FROM ?:block_links WHERE block_id IN (?n)", array('block_id', 'link_id'), array_keys($blocks_22x));
    ...
    public static function checkLicense($store_data)
    {
        $result = true;
        self::connectToOriginalDB();
        $new_license_data = self::getlicenseData();
        if (empty($new_license_data['license_number']) || !self::getLicenseStatus($new_license_data)) {
            self::connectToImportedDB($store_data);
            $old_license_data = self::getlicenseData($store_data);
            self::connectToOriginalDB();

            $result = !empty($old_license_data['license_number']) ? self::getLicenseStatus($old_license_data) : false;
        }

        return $result;
    }

    private static function getlicenseData($store_data = array())
    {
        $license_data = array(
            'current_timestamp' => self::getTimestampData($store_data),
            'license_number' => self::getLicenseNumber($store_data),
            'store_mode' => 'full',
        );
    ...
    private static function getTimestampData($store_data)
    {
        $result = '';
        if (!empty($store_data) && in_array($store_data['product_version'], array('2.2.4', '2.2.5'))) {
    ...
    private static function getLicenseNumber($store_data)
    {
        $result = '';
        if (!empty($store_data) && in_array($store_data['product_version'], array('2.2.4', '2.2.5'))) {
    ...
    private static function getLicenseStatus($license_data)
    {
        if (empty($license_data['license_number'])) {
            return false;
        }

        $result = Helpdesk::checkStoreImportAvailability($license_data['license_number'], self::VERSION_FOR_LICENSE_CHECK);
        if ($result) {
            $si_data = unserialize(Settings::instance()->getValue('si_data', 'store_import'));
    ...
    private static function backupLogos()
    {
        ...
        if (!empty($images['images_links'])) {
            $images['images_data'] = db_get_hash_array("SELECT * FROM ?:images WHERE image_id IN (?n)", 'image_id', array_keys($images['images_links']));
            $images['images_descriptions'] = db_get_hash_multi_array("SELECT * FROM ?:common_descriptions WHERE object_id IN (?n)", array('lang_code','object_id'), array_keys($images['images_data']));
        }

        return $images;
    }

    private static function restoreLogos($images)
    {
    ...
    private static function removeRussianServices($store_data)
    {
...
//            $option_exists = db_get_row("SELECT * FROM ?:settings_objects WHERE object_id = " . $setting['object_id']);
    ...
    public static function ultProcessImages($store_data)
    {
        ...
        if (self::getImportSettings('copy_images', true)) {
            $exclude_files = array('.htaccess', 'index.php');
        ...
        if (!$logos_exist) {
            ...
            foreach ($companies as $company_id) {
                ...
                self::createLogo($company_id, $layout_id, $manifest['Customer_logo']['filename'], $theme_img_path, 'theme', 'Customer_logo');
                ...
                self::createLogo($company_id, $layout_id, 'favicon.ico', $favicon_img_path, 'favicon');
                ...
                self::createLogo($company_id, 0, $manifest['Mail_logo']['filename'], $mail_img_path, 'mail', 'Mail_logo');
                if (isset($manifest['Gift_certificate_logo'])) {
                    ...
                    self::createLogo($company_id, 0, $manifest['Gift_certificate_logo']['filename'], $gc_img_path, 'gift_cert', 'Gift_certificate_logo');
                }
            }
    ...
    public static function mveProcessImages($store_data)
    {
        ...
        if (!$logos_exist) {
            ...
            self::createLogo(0, $layout_id, $root_manifest['Customer_logo']['filename'], $root_company_logo_path, 'theme', 'Customer_logo');

            $root_company_favicon_img_path = $store_data['path'] . '/skins/' . $skin_name . '/customer/images/icons/favicon.ico';
            self::createLogo(0, $layout_id, 'favicon.ico', $root_company_favicon_img_path, 'favicon');

            $root_company_mail_img_path = $store_data['path'] . '/skins/' . $skin_name . '/mail/images/' . $root_manifest['Mail_logo']['filename'];
            self::createLogo(0, 0, $root_manifest['Mail_logo']['filename'], $root_company_mail_img_path, 'mail', 'Mail_logo');
            ...
            if (isset($root_manifest['Gift_certificate_logo'])) {
                ...
                self::createLogo(0, 0, $root_manifest['Gift_certificate_logo']['filename'], $root_company_gc_img_path, 'gift_cert', 'Gift_certificate_logo');
            }
            ...
            foreach ($company_data as $company) {
                ...
                self::createLogo($company['company_id'], 0, fn_basename($theme_name), $theme_img_path, 'theme', 'Customer_logo');
                ...
                self::createLogo($company['company_id'], 0, fn_basename($mail_logo_name), $mail_img_path, 'mail', 'Mail_logo');
                ...
                if (isset($root_manifest['Gift_certificate_logo'])) {
                    ...
                    self::createLogo($company['company_id'], 0, fn_basename($gc_name), $gc_img_path, 'gift_cert', 'Gift_certificate_logo');
                }
            }
            db_query("DELETE FROM ?:common_descriptions WHERE object_holder IN ('Customer_logo', 'Mail_logo', 'Admin_logo', 'Gift_certificate_logo')");
        }
    ...
    private static function addBlock($lang_codes)
    {
        ...
        if (fn_allowed_for('ULTIMATE')) {
            ...
            $companies = array('company_id' => 0);
        }
        $new_block_ids = array();
        foreach ($companies as $company) {
            $new_block_ids[$company['company_id']] = db_query("INSERT INTO ?:bm_blocks (type, properties, company_id) VALUES ('product_filters', ?s, ?i)", serialize($block_data), $company['company_id']);
        }
        ...
        $location_ids = array();
        $block_ids = array();
        $blocks_data = db_get_array("SELECT block_id, company_id FROM ?:bm_blocks WHERE type = 'product_filters' AND properties LIKE '%blocks/product_filters/original.tpl%'");
        foreach ($blocks_data as $block) {
            $block_ids[$block['company_id']] = $block['block_id'];
        }
        ...
        if (!empty($block_ids)) {
            ...
            if (!empty($location_ids)) {
                ...
                $bc_blocks = db_get_array("SELECT block_id, company_id FROM ?:bm_blocks WHERE type = 'breadcrumbs'");

                if (!empty($bc_blocks)) {
                    foreach ($bc_blocks as $bc_block_data) {
                        ...
                        foreach ($bc_grids as $bc_grid) {
                            db_query("INSERT INTO ?:bm_snapping (block_id, grid_id, `order`) VALUES (" . $new_block_ids[$bc_block_data['company_id']] . ", " . $bc_grid['grid_id'] . ", 10)");
                        }
                    }
    ...
    private static function createLocation($languages)
    {
        ...
        foreach ($layouts as $layout) {
            ...
            $layout['company_id'] = isset($layout['company_id']) ? $layout['company_id'] : 0;
            ...
            self::createBlock('breadcrumbs', 'a:1:{s:8:\"template\";s:22:\"common/breadcrumbs.tpl\";}', $grid_id, '', '', 2, $layout['company_id']);
            self::createBlock('product_filters', 'a:1:{s:8:\"template\";s:43:\"blocks/product_filters/selected_filters.tpl\";}', $grid_id, '', '', 4, $layout['company_id']);
            ...
            self::createBlock('product_filters', 'a:1:{s:8:\"template\";s:35:\"blocks/product_filters/original.tpl\";}', $grid_id, 'blocks/wrappers/sidebox_general.tpl', '', 2, $layout['company_id']);
            ...
            self::createBlock('main', '', $grid_id, 'blocks/wrappers/mainbox_general.tpl', '', 0, $layout['company_id']);
        }
    }

    private static function createBlock($type, $properties, $grid_id, $wrapper, $user_class, $order, $company_id)
    {
        $block_id = self::isBlockExists($type, $properties, $company_id);
        if ($block_id !== false) {
    ...
    private static function isBlockExists($type, $properties, $company_id)
    {
        $cond = '';
        if (fn_allowed_for('ULTIMATE')) {
            $cond = db_quote(" AND company_id = ?i", $company_id);
        }
        $block = db_get_row("SELECT * FROM ?:bm_blocks WHERE type = '" . $type . "' AND properties = '" . $properties . "'" . $cond);
    ...
    {
        $table_prefix = self::formatPrefix();
        ...
        if (in_array($table_prefix . 'news', $tables)) {
            ...
            foreach ($company_ids as $company_id => $root) {
                $root_page_id = db_query("INSERT INTO ?:pages (parent_id, status, page_type, timestamp, company_id, position) VALUES (0, 'A', 'B', " . time() . ", " . $company_id . ", " . $pos .")");
                db_query("UPDATE ?:pages SET id_path = " . $root_page_id . " WHERE page_id = " . $root_page_id);
                db_query("REPLACE INTO ?:blog_authors (page_id, user_id) VALUES (" . $root_page_id . ", 1)");
                foreach ($languages as $language) {
                    db_query("INSERT INTO ?:page_descriptions (page_id, page, description, lang_code) VALUES (" . $root_page_id . ", 'News', '', '" . $language['lang_code'] ."')");
                }
                $company_ids[$company_id] = $root_page_id;

                if (in_array($table_prefix . 'ult_objects_sharing', $tables)) {
                    db_query("INSERT INTO ?:ult_objects_sharing (`share_company_id`, `share_object_id`, `share_object_type`) VALUES (" . $company_id . ", " . $root_page_id . ", 'pages')");
                }
            }
            foreach ($news as $news_data) {
                $pos += 100;
                $page_id = db_query("INSERT INTO ?:pages (parent_id, status, page_type, timestamp, company_id, position) VALUES (" . $company_ids[$news_data['company_id']] . ", '" . $news_data['status'] . "', 'B', " . $news_data['date'] . ", " . $news_data['company_id'] . ", " . $pos . ")");
                db_query("UPDATE ?:pages SET id_path = '" . $company_ids[$news_data['company_id']] . "/" . $page_id . "' WHERE page_id = " . $page_id);

                db_query("REPLACE INTO ?:blog_authors (page_id, user_id) VALUES (" . $page_id . ", 1)");
                ...
                foreach ($news_descriptions as $desc) {
                    db_query("INSERT INTO ?:page_descriptions (page_id, page, description, lang_code) VALUES (" . $page_id . ", '" . addslashes($desc['news']) . "', '" . addslashes($desc['description']) ."', '" . $desc['lang_code'] ."')");
                }
                if (in_array($table_prefix . 'ult_objects_sharing', $tables)) {
                    db_query("UPDATE ?:ult_objects_sharing SET share_object_id = ?i, share_object_type = 'pages' WHERE share_object_id = ?i AND share_object_type = 'news'", $page_id, $news_data['news_id']);
                    $comp_ids = db_get_fields("SELECT share_company_id FROM ?:ult_objects_sharing WHERE share_object_id = ?i AND share_object_type = 'pages'", $page_id);
                    if (!empty($comp_ids)) {
                        foreach ($comp_ids as $comp_id) {
                            db_query("REPLACE INTO ?:ult_objects_sharing (`share_company_id`, `share_object_id`, `share_object_type`) VALUES (?i, ?i, 'pages')", $comp_id, $company_ids[$news_data['company_id']]);
                        }
                    }
                }
                ...
                if (in_array($table_prefix . 'seo_redirects', $tables)) {
                    ...
                    if (!empty($seo_names)) {
                        ...
                        foreach ($languages as $language) {
                            ...
                            db_query("INSERT INTO ?:seo_redirects (src, type, object_id, company_id, lang_code) VALUES ('" . addslashes($old_url) ."', 'a', " . $page_id . ", " . $news_data['company_id'] . ", '" . $language['lang_code'] . "')");
                        }
                    }
                }

                if (in_array($table_prefix . 'discussion', $tables)) {
                    db_query("UPDATE ?:discussion SET object_id = " . $page_id . ", object_type = 'A' WHERE object_id = " . $news_data['news_id'] . " AND object_type = 'N'");
                }
            }
        ...
        if (!empty($news)) {
            foreach ($news as $news_block) {
                $condition = "page_type = 'B' AND parent_id = 0";
                if (fn_allowed_for('ULTIMATE')) {
                    $condition .= " AND company_id = " . $news_block['company_id'];
                }
                $blog_page = db_get_row("SELECT page_id FROM ?:pages WHERE " . $condition . " LIMIT 1");
    ...
    public static function productFiltersHomepage()
    {
        $blocks = db_get_array("SELECT block_id FROM ?:bm_blocks WHERE type = 'product_filters'");
        $location = db_get_row("SELECT location_id FROM ?:bm_locations WHERE dispatch = 'index.index' OR dispatch = 'index'");
        if (!empty($location)) {
            $containers = db_get_array("SELECT container_id FROM ?:bm_containers WHERE location_id = " . $location['location_id']);
            $grids = db_get_array("SELECT grid_id FROM ?:bm_grids WHERE container_id IN (" . implode(',', self::arrayColumn($containers, 'container_id'))  . ")");
            $snappings = db_get_array("SELECT snapping_id, ?:bm_blocks.block_id, company_id FROM ?:bm_snapping LEFT JOIN ?:bm_blocks ON ?:bm_blocks.block_id = ?:bm_snapping.block_id WHERE ?:bm_blocks.block_id IN (" . implode(',', self::arrayColumn($blocks, 'block_id')) .") AND grid_id IN (" . implode(',', self::arrayColumn($grids, 'grid_id')) . ")");
        }

        if (isset($snappings)) {
            $created_blocks = array();
            foreach ($snappings as $snapping) {
                if (empty($created_blocks[$snapping['company_id']])) {
                    db_query("INSERT INTO ?:bm_blocks " . self::buildFields(array(
                        'type' => 'product_filters_home',
                        'properties' => serialize(array(
                            'template' => 'blocks/product_filters/original.tpl'
                        )),
                        'company_id' => $snapping['company_id']
                    )));
                    $new_block = db_get_row("SELECT LAST_INSERT_ID() as block_id");
                    $block_id = $new_block['block_id'];

                    db_query("INSERT INTO ?:bm_blocks_descriptions (block_id, lang_code, name) SELECT " . $block_id . " as block_id, lang_code, name FROM ?:bm_blocks_descriptions WHERE block_id = " . $snapping['block_id']);
                    $created_blocks[$snapping['company_id']] = $block_id;

                    $content = db_get_array("SELECT content, lang_code FROM ?:bm_blocks_content WHERE block_id = " . $snapping['block_id']);

                    if (!empty($content)) {
                        foreach ($content as $c) {
                            $_cont = unserialize($c['content']);
                            if (!empty($_cont['items']['item_ids'])) {
                                $item_ids = implode(',', self::arrayColumn(db_get_array("SELECT filter_id FROM ?:product_filters LEFT JOIN ?:product_features ON ?:product_features.feature_id = ?:product_filters.feature_id WHERE ?:product_filters.feature_id > 0 AND ?:product_features.feature_type IN ('S', 'M', 'E') AND ?:product_filters.filter_id IN (" . $_cont['items']['item_ids'] .")"), 'filter_id'));
                            } else {
                                $item_ids = implode(',', self::arrayColumn(db_get_array("SELECT filter_id FROM ?:product_filters LEFT JOIN ?:product_features ON ?:product_features.feature_id = ?:product_filters.feature_id WHERE ?:product_filters.feature_id > 0 AND ?:product_features.feature_type IN ('S', 'M', 'E')"), 'filter_id'));
                            }
                            $_cont['items']['item_ids'] = $item_ids;

                            db_query("INSERT INTO ?:bm_blocks_content " . self::buildFields(array(
                                'block_id' => $block_id,
                                'content' => serialize($_cont),
                ...
                db_query("UPDATE ?:bm_snapping SET block_id = " . $created_blocks[$snapping['company_id']] . " WHERE snapping_id = " . $snapping['snapping_id']);
            }
        }
    }
    ...
    {
        return '(' . implode(',', array_keys($array)) . ") VALUES ('" . implode("', '", array_map('addslashes', array_values($array))) . "')";
    }

    /**
     * Updates imported tables structure to let
     * settings and addons functions work properly
     *
     * @throws StoreImportException
     *
     */
    public static function updateSystemTablesStructure()
    {
        self::connectToOriginalDB(array('table_prefix' => self::formatPrefix()));

        if (Table::exists(\Tygh::$app['db'], '?:settings_objects')) {
            $settings_table = new Table(\Tygh::$app['db'], '?:settings_objects');
            if (!$settings_table->hasColumn('parent_id')) {
                $settings_table->addColumn('parent_id', 'mediumint', array('limit' => 8, 'null' => false, 'default' => 0));
            }
        }

        if (Table::exists(\Tygh::$app['db'], '?:addons')) {
            $addons_table = new Table(\Tygh::$app['db'], '?:addons');
            if (!$addons_table->hasColumn('unmanaged')) {
                $addons_table->addColumn('unmanaged', 'tinyint', array('limit' => 1, 'null' => false));
            }
            if (!$addons_table->hasColumn('has_icon')) {
                $addons_table->addColumn('has_icon', 'tinyint', array('limit' => 1, 'null' => false));
            }
            if (!$addons_table->hasColumn('install_datetime')) {
                $addons_table->addColumn('install_datetime', 'int', array('limit' => 11, 'null' => false, 'default' => 0));
            }
        }
    ...
    /**
     * Reports successfully performed store import
     *
     * @param  array $source         Information about store imported from
     * @param  array $destination    Information about store imported to
     * @param  bool  $actualize_data True if performing actualizing data
     * @return bool  True if store import reported
     */
    public static function reportStoreImport($source, $destination, $actualize_data)
    {
        $request = array(
            'dispatch' => 'store_imports.report',
            'source' => json_encode($source),
            'destination' => json_encode($destination),
            'actualize' => $actualize_data ? 'Y' : 'N'
        );
    ...
    /**
     * Replaces imported theme and style with default ones for importing store
     *
     * @param string $theme_name Default theme of importing store
     * @param string $style_id   Default theme style
     */
    public static function replaceImportedTheme($theme_name, $style_id)
    {
        db_query("UPDATE ?:settings_objects SET value = ?s WHERE name = 'theme_name'", $theme_name);
        db_query("UPDATE ?:settings_vendor_values SET value = ?s WHERE object_id = (SELECT object_id FROM ?:settings_objects WHERE name = 'theme_name')", $theme_name);
        db_query("UPDATE ?:bm_layouts SET style_id = ?s WHERE theme_name != ?s", $style_id, $theme_name);
        db_query("UPDATE ?:bm_layouts SET theme_name  = ?s", $theme_name);
        db_query("UPDATE ?:logos SET style_id = ?s", $style_id);

        if (fn_allowed_for('ULTIMATE')) {
            $company_ids = db_get_fields("SELECT company_id FROM ?:companies");
            foreach ($company_ids as $company_id) {
                $default_layouts = db_get_fields("SELECT layout_id FROM ?:bm_layouts WHERE is_default = 1 AND company_id = ?i", $company_id);
                if (count($default_layouts) > 1) {
                    db_query("UPDATE ?:bm_layouts SET is_default = 0 WHERE company_id = ?i", $company_id);
                    db_query("UPDATE ?:bm_layouts SET is_default = 1 WHERE layout_id = ?i AND company_id = ?i", reset($default_layouts), $company_id);
                }
            }
        } else {
            $default_layouts = db_get_fields("SELECT layout_id FROM ?:bm_layouts WHERE is_default = 1");
            if (count($default_layouts) > 1) {
                db_query("UPDATE ?:bm_layouts SET is_default = 0");
                db_query("UPDATE ?:bm_layouts SET is_default = 1 WHERE layout_id = ?i", reset($default_layouts));
            }
        }
    }

    /**
     * Returns all store import settings or a single setting value
     *
     * @param null    $setting_name Setting name
     * @param null    $default      Default value to return when setting is missing
     * @param boolean $switch_db    Change database before getting settings
     *
     * @return mixed
     *     array of store import settings if $setting_name is not set
     *     value of setting if $setting_name is found in settings
     *     $default if $setting_name is not found in settings
     */
    public static function getImportSettings($setting_name = null, $default = null, $switch_db = true)
    {
        if ($switch_db) {
            self::connectToOriginalDB();
        }
        ...
        if (isset($setting_name)) {
            if (array_key_exists($setting_name, $si_data)) {
                $value = $si_data[$setting_name];
            } else {
                $value = $default;
            }
        } else {
    ...
    public static function logError()
    {
        ...
        $type = reset($args);
        if ($type === self::ERROR_SQL) {
            list($type, $error_text) = $args;
        } else {
            if (ini_get('error_reporting') == 0) {
                return;
            }
            $type = self::ERROR_PHP;
            // when calling the function manually, drop first argument as it is type of error
            if ($args[0] == $type) {
                array_shift($args);
            }
            // compatible with set_error_hadler parameters
            list($errno, $error_text, $filename, $line) = $args;
        }
        if (!empty($error_text)) {
            if ($type == self::ERROR_SQL) {
                fn_set_progress('echo', self::getUnavailableLangVar(
                    'sql_error',
                    array(
            ...
            fn_set_notification(
                'E',
                __('error'),
                self::getUnavailableLangVar('errors_during_import')
            );
        }
    }
    ...
    public static function shutdown()
    {
        ...
        if ($error !== NULL && $error['type'] === E_ERROR) {
            self::logError(self::ERROR_PHP, $error['type'], $error['message'], $error['file'], $error['line']);
        }
    }

    /**
     * Checks if required process step can be accessed
     *
     * @param int $step Required step
     *
     * @return array (
     *   boolean $result True if step can be accessed
     *   int     $step   Valid step that can be accessed
     * )
     */
    public static function checkStep($step = 1)
    {
        $result = true;
        $old_step = self::getImportSettings('step', 1, false);
        if ($step - $old_step > 1) {
            $result = false;
            $step = $old_step;
        }
