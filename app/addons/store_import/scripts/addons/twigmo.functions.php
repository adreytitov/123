...
function fn_twigmo_remove_bm()
{
    ...
    if (!empty($location_ids)) {
        db_query("DELETE FROM `?:bm_locations` WHERE `location_id` IN (?n)", $location_ids);
        db_query("DELETE FROM `?:bm_locations_descriptions` WHERE `location_id` IN (?n)", $location_ids);
    }
}
